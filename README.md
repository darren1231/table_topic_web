...
第一次分析會沿用既有教練回覆中的具體改善建議，整理成 2–3 個 `retryGoals`，不改變原本完整報告的 AI 輸出格式，也不會為目標增加額外 AI 呼叫。使用者再答同一題時，第二筆歷史紀錄以 `parentAttemptId` / `retryOf` 指向前一次回答，並以 `sessionId` 保留同一練習脈絡；舊紀錄會視為獨立的第一次回答。
